...
describe('Form.DatePicker', () => {
  ...
  test('should shows correct picker',  async () => {
    const { rerender } = render(<DatePicker value={new Date()} onChange={() => null} picker={Picker.day} />)
  ...
  test('should chooses today day after click on Today', async () => {
    ...
    const { rerender } = render(<DatePicker value={value} onChange={onChange} picker={Picker.day} />)
    ...
    expect([value.getFullYear(), value.getMonth(), value.getDate()]).toStrictEqual([today.getFullYear(), today.getMonth(), today.getDate()])
  })
