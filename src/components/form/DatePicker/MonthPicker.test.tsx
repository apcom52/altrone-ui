...
describe('Form.DatePicker', () => {
  ...
  test('MonthPicker should renders correctly', () => {
    ...
    render(<MonthPicker currentMonth={date} selectedDate={date} onChange={() => null} minYear={1900} maxYear={2050} />)
  ...
  test('MonthPicker should show years correctly', () => {
    const date = new Date(2022, 9, 1)
    render(<MonthPicker currentMonth={date} selectedDate={date} onChange={() => null} minYear={2000} maxYear={2030} />)
  ...
  test('MonthPicker should change date correctly', async () => {
    ...
    const { rerender } = render(<MonthPicker currentMonth={value} selectedDate={value} onChange={onChange} minYear={2000} maxYear={2030} />)

    const june = screen.getByText('June')
    const year2025 = screen.getByText('2025')

    await waitFor(() => fireEvent.click(june))
    rerender(<MonthPicker currentMonth={value} selectedDate={value} onChange={onChange} minYear={2000} maxYear={2030} />)
