...
const Checkbox = ({ disabled, id, checked = false, danger = false, children, CheckIconComponent, className, onChange, hintText, errorText, ...props }: CheckboxProps) => {
  const generatedCheckboxId = useId()

  const checkboxId = id || generatedCheckboxId

  return <BasicInput hintText={hintText} errorText={errorText} disabled={disabled}>
    <label htmlFor={checkboxId} className={clsx('alt-checkbox', className, {
