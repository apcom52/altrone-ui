import {ContextAction, ContextMenuType as ContextMenuType, ParentContextAction} from "../../../types";
...
const ContextMenu = ({ menu, onClose }: ContextMenuComponentProps) => {
  ...
  return <div className='alt-context-menu-list'>
    ...
    {!selectedParentItem && menu.map((item, itemIndex) => (
      'onClick' in item ? <ContextMenuItem key={itemIndex} {...item} onClick={() => onActionClick(item)} /> : <ContextParentMenuItem key={itemIndex} onClick={onParentItemClick} onClose={onClose} {...item} />
    ))}
  </div>
}
