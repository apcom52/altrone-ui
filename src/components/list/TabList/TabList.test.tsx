...
describe('List.TabList', () => {
  ...
  test('should variants works correctly', () => {
    const { rerender } = render(<TabList tabs={TABS} selected={1} onChange={() => null} variant={TabListVariant.border} />)
    let tabList = screen.getByTestId('alt-test-tab-list')

    expect(tabList).toHaveClass('alt-tab-list--variant-border')

    rerender(<TabList tabs={TABS} selected={1} onChange={() => null} variant={TabListVariant.solid} />)
  ...
  test('should AddTab button works correctly', async () => {
    ...
    render(<TabList tabs={TABS} selected={1} onChange={() => null} variant={TabListVariant.solid} onAddTab={onAdd} />)
  ...
  test('should CloseTab button works correctly', async () => {
    ...
    const { rerender } = render(<TabList tabs={tabs} selected={1} onChange={() => null} variant={TabListVariant.solid} onCloseTab={onClose} />)
    let removeButton = screen.queryAllByTestId('alt-test-tab-close')

    await waitFor(() => fireEvent.click(removeButton[0]))

    rerender(<TabList tabs={tabs} selected={1} onChange={() => null} variant={TabListVariant.solid} onCloseTab={onClose} />)
